...
AnywhereMonitor::AnywhereMonitor(const DeviceContext& device)
    : device(device), pacer(make_presentation_pacer(device)),
      monitor_worker(std::bind_front(&AnywhereMonitor::do_monitor, this)) {}

AnywhereMonitor::~AnywhereMonitor() {}

void AnywhereMonitor::enqueue_work(
    ...
    if (device.instance.layer.should_debug) {
        std::fprintf(stderr, "[LowLatency] Anywhere Monitor: enqueue_work — %zu span(s)\n",
                     spans.size());
    }
...
void AnywhereMonitor::do_monitor(const std::stop_token stoken) {
    for (;;) {
        auto lock = std::unique_lock{mutex};
        cv.wait(lock, stoken,
        ...
        if (device.instance.layer.should_debug) {
            const auto gpu_ms = std::chrono::duration<double, std::milli>(
                timing.gpu_end - timing.gpu_start);
            std::fprintf(stderr, "[LowLatency] Anywhere Monitor: gpu_work=%.2fms spans=%zu\n",
                         gpu_ms.count(), spans.size());
        }
...
void AnywhereMonitor::poll_feedback_locked() {
...
void AnywhereMonitor::set_swapchain(VkSwapchainKHR sc) {
    swapchain = sc;
    pacer->set_swapchain(sc);
}

void AnywhereMonitor::set_present_mode(VkPresentModeKHR mode) {
    pacer->set_present_mode(mode);
}

bool AnywhereMonitor::feed_present_feedback(std::uint64_t target_ns,
